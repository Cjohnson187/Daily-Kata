...
public class KeyCardAlert {
	...
	public List<String> find(String[] keyName, String[] keyTime) {
		...
		if (keyName.length <= 1 || keyTime.length <= 1 || keyName.length != keyTime.length) {
			return results;
		}
		// build map
		System.out.println("log");
		Map<String, List<String>> keyLog = new HashMap<>();
		for (int i = 0; i < keyName.length; i++) {
			if(keyLog.containsKey(keyName[1])) {
				keyLog.get(keyName[i]).add(keyTime[i]);

			} else {
				keyLog.put(keyName[i], new ArrayList<String>());
				keyLog.get(keyName[i]).add(keyTime[i]);
			}
		}
		System.out.println("log2");
		// check map
		for (Map.Entry<String, List<String>> entry : keyLog.entrySet()) {
			if(checkLogs(entry.getValue()))
	...
	public boolean checkLogs(List<String> logs) {
		
		Collections.sort(logs);
		for(String s : logs) {
			System.out.println("time in list = " + s);
		}
